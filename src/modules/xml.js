...
const perform_operation_on_xml = async (xml_path) => {
    ...
    try {
        const xml_string = fs.readFileSync(`${config.read_file_path_from_root}/${xml_path}`, 'utf-8');
        await postToTaxDeparment(xml_string, current_path);
        const complete_path = path.join(config.move_completed_file_path, path.basename(xml_path));
        ...
        const failed_path = path.join(config.move_failed_file_path, path.basename(xml_path));
...
const operate_xml = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        logger.info(`Operation completed on file - ${files[i]} (${i + 1}/${files.length})`);
    }
}
